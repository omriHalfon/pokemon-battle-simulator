...
/**
 * @brief Clears the input buffer to prevent leftover characters from interfering.
 * Why we made it: Handles cases where input functions like `scanf` leave trailing
 * characters (e.g., newline `\n`) in the input stream, which could disrupt subsequent input operations.
 */
...
/* ------------------------------------------------------------
   ...
   ------------------------------------------------------------ */
/**
 * @brief Adds a new owner to the circular linked list with a given starter Pokémon.
 * @param ownerName Dynamically allocated string representing the owner's name.
 * @param starter Pointer to the root of the owner's starter Pokémon Pokedex tree.
 * Why we made it: Provides a streamlined way to create a new owner with a starter Pokémon
 * and link them into the circular list of owners, ensuring proper initialization and structure.
 */
...
/**
 * @brief Checks if a Pokémon with the specified ID exists in the BST.
 * @param root Pointer to the root of the Pokémon BST.
 * @param id The ID of the Pokémon to search for.
 * @return 1 if the Pokémon exists in the tree, 0 otherwise.
 * Why we made it: Provides a simple and efficient way to verify the existence of a Pokémon
 * in the BST, useful for preventing duplicates or validating operations like removal or evolution.
 */
...
/* ------------------------------------------------------------
   ...
   ------------------------------------------------------------ */
/**
 * @brief Calculates the total power of a Pokémon based on its stats.
 * @param id The ID of the Pokémon in the pokedex array.
 * @return The calculated total power as a double.
 * Why we made it: Provides a weighted formula to evaluate a Pokémon's overall strength
 * by combining its attack and HP, allowing for meaningful comparisons and battle logic.
 */
...
/**
 * @brief Checks if an owner's name exists in the circular linked list of owners.
 * @param name The name of the owner to search for.
 * @return 1 if the name exists in the list, 0 otherwise.
 * Why we made it: Ensures uniqueness of owner names when adding new owners
 * and allows quick validation for operations involving specific owners.
 */
...
/* ------------------------------------------------------------
   ...
   ------------------------------------------------------------ */
/**
 * @brief Displays a numbered list of all existing owners and their Pokedexes.
 * Why we made it: Provides a user-friendly way to view all owners in the circular
 * linked list, making it easier to choose an owner for operations like deletion,
 * merging, or traversal.
 */
...
/**
 * @brief Counts the total number of Pokedexes (owners) in the circular linked list.
 * @return The total count of Pokedexes as an integer.
 * Why we made it: Provides a quick and reliable way to determine the number of
 * owners, which is useful for validating operations like merging, deletion,
 * or choosing from a list of Pokedexes.
 */
...
/**
 * @brief Merges all unique Pokémon from the second BST into the first BST.
 * @param first Pointer to the pointer of the root of the first BST (destination).
 * @param second Pointer to the root of the second BST (source).
 * Why we made it: Allows combining two Pokedex trees by adding all unique Pokémon
 * from the second tree into the first while avoiding duplicates, demonstrating tree traversal and dynamic updates.
 */
...
/* ------------------------------------------------------------
   ...
   ------------------------------------------------------------ */
/**
 * @brief Prints owner names in forward direction starting from the head.
 * @param numOfPrints The number of names to print.
 * Why we made it: Provides a user-friendly way to traverse and display
 * owner names in a circular list in a forward sequence.
 */
void printForward(int numOfPrints);

/**
 * @brief Prints owner names in backward direction starting from the head.
 * @param numOfPrints The number of names to print.
 * Why we made it: Enables reverse traversal of the circular list for user
 * convenience and to demonstrate backward navigation in linked structures.
 */
